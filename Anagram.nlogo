...
to-report explode [w]
  if (length w) = 0 [report []]
  report fput first w explode butfirst w
end

to-report implode [w]
  if (length w) = 0 [report ""]
  report word implode butlast w last w
end

to-report signature [string]
  report implode sort-by < explode string
end

to read-file [file-name contents] ; Jeweils eine ganze Datei bearbeiten
  foreach split contents ; Zeile lesen und zerteilen
    [
      mapred:emit signature ? ? ; <wort, 1> Wort kommt aus split
    ]
end
...
to anagram [key values]
  let words []
  
  foreach values [set words fput ? words]
  
  mapred:emit key words
end
...
to wc
  ...
  mapred:mapreduce task read-file task anagram
  foreach sort-by [length butfirst ?1 < length butfirst ?2] mapred:result
  [
    show ? 
  ]
end
